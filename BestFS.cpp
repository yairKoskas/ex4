...
Solution BestFS::search (Searchable searchable) const {
    const State state = State(searchable.getInitialState().getRow(), searchable.getInitialState().getCol(), 0,  nullptr);
    pq.push(state);
...
Solution BestFS::backTrace() const {
    ...
    while(true){
        ...
        while(!*(closed.end()).equals(*(solu.begin()->lastStateBeforeCurrent()))){
            closed.pop();
        }
    }
}
